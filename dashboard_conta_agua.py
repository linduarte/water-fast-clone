...
if st.button("🚀 Calcular"):
    ...
    if df.empty:
        st.warning("Nenhum apartamento válido encontrado. Verifique os valores informados no painel lateral.")
        # still show total account value
        st.metric("💰 Valor total da conta", format_currency(resultado["valor_total_da_conta"]))
    else:
        # Métricas
        col1, col2, col3 = st.columns(3)
        col1.metric("🔢 Valor fixo por apto", format_currency(resultado["valor_fixo_corrigido"]))
        col2.metric(
            "👤 Valor variável por residente",
            format_currency(resultado["valor_variavel_por_residente"]),
        )
        col3.metric("💰 Valor total da conta", format_currency(resultado["valor_total_da_conta"]))
